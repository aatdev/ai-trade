...
**Журнал:** `results/analysis/signals.md` (N сигналов прочитано)
...
- **Никогда не пиши в `signals.md`** — этот скилл только читает журнал.
...
- **Временные файлы** (`./tmp/signals_plan_*.json`) — не финальный артефакт; в `results/` их не клади.

## Связанные файлы

- `results/analysis/signals.md` — источник сигналов (читается).
