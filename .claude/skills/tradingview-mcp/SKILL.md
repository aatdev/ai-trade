...
description: Read and control a live TradingView Desktop chart via the TradingView MCP (CDP port 9222). Use whenever the user asks about their chart, current price/quote, indicator values, OHLCV data, Pine Script editing/compilation, replay-mode practice, drawings, alerts, multi-symbol screening with batch_run, switching symbol/timeframe/chart-type, scrolling to dates, or any task that calls one of the `mcp__tradingview__*` tools. Loads tool-selection decision tree, context-management rules, output-size estimates, and tool conventions to avoid context bloat.
---
