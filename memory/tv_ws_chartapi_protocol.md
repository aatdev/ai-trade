...
Та же правка распространена на скринеры ([[cts_tv_data_layer]]): CLI-команда `tv symbol` получила флаг `--nowait` (`src/cli/commands/chart.js` → `setSymbol({wait:false})`), а `scripts/lib/tv_client_base.py._fetch_bars` теперь зовёт `tv symbol X --nowait`. Живой путь скринеров (он вторичен — cache-first через metrics_cache) ~13с→~2.8с на символ; дальше доминирует `SETTLE=2.0с` в базе (его `--nowait` не убирает, т.к. settle ждёт загрузки баров после переключения). Node `parseArgs` не умеет `--no-foo`, поэтому флаг именно `--nowait`.
